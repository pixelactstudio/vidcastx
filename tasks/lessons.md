...
- **Bot PRs opened with `GITHUB_TOKEN` wait for approval, and dispatched runs don't count.** The repo's first-time-contributor policy parks the release PR's CI in `action_required`. A `workflow_dispatch` run on the same commit is linked to the PR but doesn't satisfy the required `CI` check; approving the parked run does. Check `gh pr checks <n> --required` before claiming a check is satisfied.
